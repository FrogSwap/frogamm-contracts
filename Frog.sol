...
contract Frog is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0x96D1ea81D41111Ad653BC78F6e7D4FE90c8FBA8a;
    ...
    // Min amount to liquify. (default 10 FROGs)
    ...
    // The operator can call all functions except of minting FROG
    ...
    /**
     * @notice Constructs the $FROG contract.
     */
    constructor() public BEP20("FROG", "FROG") {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (FrogChef).
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && swapPair != address(0)
            && sender != swapPair           // excludes Buying in KEK Liquidity
            && sender != owner()            // excludes owner (later FrogChef)
        ) { swapAndLiquify(); }
        ...
        transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "FROG::transfer: Burn value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the frog -> weth path for swapping
    ...
    // To receive KEK from swapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "FROG::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "FROG::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "FROG::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 400, "FROG::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "FROG::updateSwapRouter: Invalid pair address.");
...
// FrogChef is the master of FROG. He can make Frog and he is a fair guy.
//
...
contract FrogChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardSecond; // Last second that FROGs distribution occurs.
    ...
    // FROG tokens created per second.
    ...
    // The block timestamp when Frog mining starts.
    ...
    function pendingFrog(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 frogReward = multiplier
                .mul(FrogPerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
                uint256 devReward = frogReward.div(10);
                uint256 totalRewards = frog.totalSupply().add(devReward).add(
                frogReward
            );
                if (totalRewards > frog.maxSupply()) {
                frogReward = frog.maxSupply().sub(frog.totalSupply());
            }
            accFrogPerShare = accFrogPerShare.add(
                frogReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accFrogPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= frog.maxSupply()) {
            ...
            frog.mint(devaddr, frogReward.div(10));
            frog.mint(address(this), frogReward);
        } else {
            // mint the difference only to MC, update frogReward
            frogReward = frog.maxSupply().sub(frog.totalSupply());
            frog.mint(address(this), frogReward);
        }

        if (frogReward != 0) {
            // only calculate and update if frogReward is non 0
    ...
    // Deposit LP tokens to FrogChef for Frog allocation.
    ...
    // Withdraw LP tokens from FrogChef.
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accFrogPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeFrogTransfer(msg.sender, pending);
    ...
    //Frog has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _FrogPerSecond) external onlyOwner {
        require(_FrogPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        FrogPerSecond = _FrogPerSecond;
